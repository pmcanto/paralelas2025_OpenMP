...
int main(int argc, char *argv[]) {
    // 1. Setup (Idêntico ao Sequencial)
    ...
    // 2. Processamento Paralelo (CRITICAL)
    printf(">> Iniciando processamento PARALELO (CRITICAL) com %d threads...\n", omp_get_max_threads());
    
    double start_time = omp_get_wtime();

    #pragma omp parallel for
    for (long i = 0; i < total_lines; i++) {
        // A diretiva critical cria um gargalo. Apenas 1 thread entra aqui por vez.
        // Isso protege a tabela inteira (leitura E escrita).
        #pragma omp critical
        {
